...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(
        ...
        default_qresi_counts=0, quant_resi=0.5, share_quant_resi=4,  # share_quant_resi: args.qsr
    ):
        super().__init__()
        ...
        if share_quant_resi == 0:   # non-shared: \phi_{1 to K} for K scales
            self.quant_resi = PhiNonShared([(Phi(Cvae, quant_resi) if abs(quant_resi) > 1e-6 else nn.Identity()) for _ in range(default_qresi_counts)])
    ...
    def extra_repr(self) -> str:
        return f'znorm={self.using_znorm}, beta={self.beta}, quant_resi={self.quant_resi_ratio}'
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if self.using_znorm:
                ...
                if self.training and dist.initialized():
                    handler.wait()
                    if self.record_hit == 0: self.ema_vocab_hit_SV[0].copy_(hit_V)
                    elif self.record_hit < 100: self.ema_vocab_hit_SV[0].mul_(0.9).add_(hit_V.mul(0.1))
                    else: self.ema_vocab_hit_SV[0].mul_(0.99).add_(hit_V.mul(0.01))
                    self.record_hit += 1
    ...
    def f_to_idxBl_or_fhat(self, f_BChw: torch.Tensor, to_fhat: bool) -> List[Union[torch.Tensor, torch.LongTensor]]:  # z_BChw is the feature from inp_img_no_grad
        assert not to_fhat
        ...
        return idx_N.reshape(B, -1)
    
    # ===================== idxBl_to_var_input: only used in VAR training, for getting teacher-forcing input =====================
    def idxBl_to_var_input(self, gt_ms_idx_Bl: torch.Tensor) -> torch.Tensor:
        next_scales = []
        B = gt_ms_idx_Bl.shape[0]
        C = self.Cvae
        H = W = int(gt_ms_idx_Bl.shape[1]**0.5)
